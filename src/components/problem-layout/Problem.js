...
export const updateCanvas = async (context, mastery, components) => {
    if (context.jwt) {
        ...
        if (err || !response) {
            ...
            switch (response.status) {
                case 400:
                    ...
                    const [message, ...addInfo] = responseText.split("|");
                    switch (message) {
        ...
        const { getByKey, setByKey } = context.browserStorage;
        const showWarning =
...
class Problem extends React.Component {
    ...
    handleUpdateCanvas = (mastery, components) => {
        updateCanvas(this.context, mastery, components);
    };
    ...
    answerMade = async (cardIndex, kcArray, isCorrect, stepId) => {
        ...
        if (!this.context.debug && isCorrect) {
            ...
            this.handleUpdateCanvas(0.0, relevantKc);
        } 
